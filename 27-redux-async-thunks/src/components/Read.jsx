...
const Read = () => {
  ...
  return (
    <div>
      ...
      <h2>All data</h2>
      <input className="form-check-input" name="gender" checked={radioData === ""} type="radio" onChange={() => setRadioData("")} />
      ...
      <div>
        {users
          .filter((ele) => (searchData.length === 0 ? true : ele.name.toLowerCase().includes(searchData.toLowerCase())))
          .filter((ele) => (radioData === "" ? true : ele.gender === radioData))
          .map((ele) => (
            <div key={ele.id} className="card w-50 mx-auto my-2">
              <div className="card-body">
                ...
                <button className="card-link" onClick={() => dispatch(deleteUser(ele.id))}>Delete</button>
              </div>
            </div>
          ))}
      </div>
